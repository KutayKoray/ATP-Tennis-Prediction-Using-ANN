...
print("--- 'Premium Yakıt' Üretim Script'i Başlatıldı (1990+ Verisi) ---")
print("--- Elo, H2H ve Momentum hesaplanacak. ---")
...
output_npz_file = 'processed_data_V2_ELO_1990plus.npz' # YENİ DOSYA ADI
...
try:
    ...
    df_all['winner_ht'].fillna(df_all['w_ht_fill'], inplace=True)
    # Loser
    ...
    df_all['loser_ht'].fillna(df_all['l_ht_fill'], inplace=True)
...
# --- 3. Veriyi Sıralama ve FİLTRELEME (Kritik!) ---
df_all['tourney_date'] = pd.to_datetime(df_all['tourney_date'], format='%Y%m%d')

# YENİ ADIM: Sadece 1990 ve sonrasındaki verileri tut
print(f"   ... Orijinal veri boyutu (1968-2024): {len(df_all)} maç")
df_all = df_all[df_all['tourney_date'] >= '1990-01-01'].copy()
print(f"   ... YENİ veri boyutu (1990-2024): {len(df_all)} maç")
...
# Train (1990-2022), Val (2023), Test (2024)
df_all['dataset_marker'] = 'train' 
df_all.loc[df_all['tourney_year'] == 2023, 'dataset_marker'] = 'val'
df_all.loc[df_all['tourney_year'] >= 2024, 'dataset_marker'] = 'test' 
...
# --- 1. Elo K-Faktörü Fonksiyonu ---
def get_elo_k_factor(matches_played):
    return 250 / ((matches_played + 5) ** 0.4)

# --- 2. Hafıza (Bellek) Sözlüklerini Başlat ---
# (Elo, H2H, vb. 1990 öncesi oyuncular için 1500'den başlayacak, bu normaldir)
elo_ratings = defaultdict(lambda: 1500) 
player_stats = defaultdict(lambda: {
    'matches_played': 0, 'wins': 0,
    'surface_matches': defaultdict(int), 'surface_wins': defaultdict(int),
    'last_10_matches_won': deque(maxlen=10), 
    'last_10_matches_ace_rate': deque(maxlen=10), 
    'last_7_days_minutes': deque() 
})
...
for row in df_all.itertuples(index=True):
    ...
    for date, mins in list(w_stats['last_7_days_minutes']):
        if (current_date - date).days <= 7: w_total_mins += mins
        else: w_stats['last_7_days_minutes'].popleft() 
    w_fatigue_7d_mins[i] = w_total_mins
    l_total_mins = 0
    for date, mins in list(l_stats['last_7_days_minutes']):
        if (current_date - date).days <= 7: l_total_mins += mins
        else: l_stats['last_7_days_minutes'].popleft()
    ...
    K_w, K_l = get_elo_k_factor(w_matches_played), get_elo_k_factor(l_matches_played)
    elo_ratings[w_id] = w_elo_pre_match[i] + K_w * (1 - E_w)
    elo_ratings[l_id] = l_elo_pre_match[i] + K_l * (0 - (1 - E_w))
    
    w_stats['matches_played'] += 1; w_stats['wins'] += 1
    w_stats['surface_matches'][surface] += 1; w_stats['surface_wins'][surface] += 1
    ...
    if w_id == p1_in_key: h2h_data['p1_wins'] += 1
...
print("\n--- İŞLEM TAMAMLANDI ---")
print(f"Tüm veriler (Elo, H2H, Momentum, 1990+) '{output_npz_file}' dosyasına kaydedildi.")
